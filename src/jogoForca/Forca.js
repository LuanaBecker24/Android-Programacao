...
export default function Forca(props) {
  ...
  const handleAdivinhar = () => {
    ...
    if (!/[a-zA-Z]/.test(letra) || letra.length !== 1) {
      return <Text style={styles.errorMessage}>Digite uma única letra válida</Text>;
    }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.forcaContainer}>
        {linhas.map((letra, index) => (
          <Text key={index} style={styles.letra}>{letra}</Text>
        ))}
      </View>
      ...
      {!continuaJogo && (
        <Text style={styles.perdeu}>Não foi dessa vez! 🤡 A palavra era "{palavra}"</Text>
      )}
    </View>
